...
async function loadSources() {
  const configDir = join(SCRIPT_DIR, "..", "config");

  const sourcesPath = join(configDir, "investment-sources.json");
  const metadataPath = join(configDir, "investment-metadata.json");

  const sources = JSON.parse(
    await readFile(sourcesPath, "utf-8")
  );
  ...
  try {
    ...
    console.warn(
      "investment-metadata.json not found, continuing without metadata."
    );
  }
...
async function fetchXContent(xAccounts, bearerToken, state, errors,metadata) {
  const results = [];
  ...
  for (const account of xAccounts) {
    ...
    try {
      ...
      const info = metadata[account.handle] || {};
      results.push({
        source: "x",
        name: account.name,
...
if (runPodcasts && !pod2txtKey) {
  ...
  const { sources, metadata } = await loadSources();
  ...
  if (runTweets) {
    ...
    await writeFile(
      join(SCRIPT_DIR, "..", "feed-investment-x.json"),
      JSON.stringify(xFeed, null, 2),
    );
    console.error(
      `  feed-investment-x.json: ${xContent.length} builders, ${totalTweets} tweets`,
    );
  }
